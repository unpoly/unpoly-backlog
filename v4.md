...
- Consider restoring moveBefore() support
  - Issue 426866831
...
- Can we join focus and scroll options, but still offers exceptions for the more complicated scroll?
  - { highlight: '#target' }
  - { attention: '#target' }
  - How does this work with the sequence of auto scroll strategies?
  - There is already some overlap between scrolling and focusing
    - revealHash() takes a { focus: true } option
    - focus() scrolls by default, except with { preventScroll: true }
- Consider no longer rejecting on failed responses, force users to check #ok
  - IF it makes our own code easier

  
Clean up
--------

- Replace the _up_method cookie with a `<meta name="request-method">`
- Rename autosubmit to auto-submit
...
- Be more explicit about request props
  ...
  - request.mode => request.renderLayerMode, X-Up-Layer-Mode
    - But we might want X-Up-Mode="frame" later? Or is this something separate?
- Remove remaining jQuery support
- Consider moving .up-scrollbar-away to html instead of body (v4?)
  => We actually set it at every shifted element, and also set --up-original-xxx props
- Consider moving event options for up.emit() into a third argument
- accept() / dismiss() Consider moving the value to { value }
- Remove Rails UJS workarounds (up.rails)
- Consider renaming the popup mode to something else
  - It shouldn't be a popover, since it is built to include interactive content, is that right terminology?
- Consider up.on callbacks no longer passing the element or data as additional arguments


Strong candidates
-----------------

- Consider removing event aliasing (watchInputEvents, watchChangeEvents)

- Check if we can use Document.parseHTMLUnsafe() instead of DOMParser#parseFromString()
...
- Replace usage of e.paint() with @starting-style
  - https://caniuse.com/mdn-css_at-rules_starting-style (good support)
...
- Is `scrollbar-gutter: stable` a replacement for up.BodyShifter?
  - https://caniuse.com/mdn-css_properties_scrollbar-gutter (good support)
...
- Replace `up.URLPattern` with standard `URLPattern`
  - In TP in Safari: https://caniuse.com/mdn-api_urlpattern
...
- Consider removing layer context
  ...
  - Poll users
- Now that we have native :has(), can we get replace up.Selector with CSS selector expansion?
...
- Address maintenance burden with popup overlays
  ...
  - Consider implementing with `[popover]`
    - https://caniuse.com/mdn-api_htmlelement_popover (good support)
...
- Automatic `up.hello()` with `MutationObserver`
  ...
  - Needs to be async?
    ...
    - We will see flashes of uncompiled content
  - Performance?
