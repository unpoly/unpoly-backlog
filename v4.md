...
- Can we replace or extend { feedback } with a generalized optimistic rendering strategy?
  - How to consolidate exceptions?
    - e.g. server renders a failed response, and that goes into another layer
    - e.g. server changes target
  - Manual callbacks?
    - onTargetChanged
    - onLayerChanged
  - Auto-capture state that we can restore?
...
- Consider deprecating commas as multi-fragment separators.
  - Possibly replace with `and`
  - Since `,` also means alternative
